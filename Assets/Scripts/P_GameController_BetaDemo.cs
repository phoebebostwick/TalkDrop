...
public class P_GameController_BetaDemo : MonoBehaviour {
...
//	public AudioClip Clip;
	...
	public Image recordingIconFill;
	public float recordingCurrentLength;
	...
	void Update() {
		if (Microphone.IsRecording (null)) {
			...
			recordingIconFill.fillAmount = recordingCurrentLength;
			Debug.Log(Microphone.GetPosition(null));
			if (Microphone.GetPosition (null) >= 110000) {
				StopRecording ();
			}
		}
	}
	...
	public void StopRecording () {
		...
		recordingIconFill.fillAmount = 0;
		recordingCurrentLength = 0;
		P_ClipData tempClipData;
	...
	public GameObject AddShape(Vector3 ShapePosition, Quaternion ShapeRotation, P_ClipData clip) {
...
//			Debug.Log ("parse json " + byteToFloatArray.Length); //110250
			...
			ShowShapes (ShapePosition, Quaternion.identity, byteToFloatArray);
		}
...
//				Debug.Log ("get instance " + www.downloadHandler.text);
	...
	public GameObject ShowShapes(Vector3 ShapePosition, Quaternion ShapeRotation, float[] samples) {
		...
		//create clip
		tempClip = AudioClip.Create ("clipname", samples.Length / 1, 1, 11025, false);
		tempClip.SetData (samples, 0);
		newAS.clip = tempClip;
		Debug.Log("Show shapes Clip with samples: " + samples.Length);
