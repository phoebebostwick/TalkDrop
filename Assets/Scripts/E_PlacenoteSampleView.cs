...
public class E_PlacenoteSampleView : MonoBehaviour, PlacenoteListener
{
	...
	void Start ()
	{
		...
		if (LibPlacenote.Instance.Initialized ()) {
			//if savedmap id is null run newMap
			OnNewMapClick ();
			//else run load map
		}
...
//		FeaturesVisualizer.clearPointcloud();
		GetComponent<GameController_BetaDemo>().ClearShapes();
	...
	public void OnLoadMapClicked ()
	{
		...
		ResetSlider ();
		//THIS NEEDS TO BE SAVED MAP ID AND NEEDS TO BE CALLED ON START
...
//				mLabelText.text = "Saved Map Name: " + metadata.name;
				...
				JObject shapeList = GetComponent<GameController_BetaDemo>().Shapes2JSON();
...
//			mLabelText.text = "Localized";
			GetComponent<GameController_BetaDemo>().LoadShapesJSON (mSelectedMapInfo.metadata.userdata);
...
//			mLabelText.text = "Searching for position lock";
			...
			if (GetComponent<GameController_BetaDemo>().shapeObjList.Count != 0) {
				GetComponent<GameController_BetaDemo>().ClearShapes ();
			}
		}
	}
